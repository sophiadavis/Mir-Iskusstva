...
def main():
    ...
    with open('socialistRealist.csv', 'wb') as f: # use 'a' if this takes forever and we have problems 
        ...
        for jpg in jpgs:
            ...
            avgColor = subprocess.check_output(["convert", jpg, "-scale", "1x1\!", "-format", "%[fx:int(255*r+.5)],%[fx:int(255*g+.5)],%[fx:int(255*b+.5)]", "info:"])
            ...
            redHistRaw = subprocess.check_output(["convert", jpg, "-channel", "R", "-separate", "-format", "%c", "histogram:info:"])            
            greenHistRaw = subprocess.check_output(["convert", jpg, "-channel", "G", "-separate", "-format", "%c", "histogram:info:"])
            blueHistRaw = subprocess.check_output(["convert", jpg, "-channel", "B", "-separate", "-format", "%c", "histogram:info:"])
            ...
            attrs = ["SocRealism", jpg, numColors, avgColor] + grayBins + redBins + greenBins + blueBins
            print attrs
