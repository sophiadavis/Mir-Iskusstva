...
p + geom_line(aes(col = ind, group = ind)) + scale_color_manual(values = pal, breaks = c("X1", "X2", "X3", "X4", "X5", "X6", "X7", "X8", "X9", "X10", "X11", "X12", "X13", "X14", "X15"), name = "Model", labels = c(1:15)) + stat_summary(data = final, fun.y = "min", aes(group = ind, label = ind), geom = "text", size = 2, vjust = 1.5, hjust = -1, family = "mono") + theme(text = element_text(family = "mono")) + labs(x = "Iteration", y = "MSE", title = "MSE vs Number of Iterations, Logistic Activation Function", col = "Combination") 
